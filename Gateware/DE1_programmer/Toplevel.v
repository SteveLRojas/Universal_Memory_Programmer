...
// 0xFFC0 to 0xFFCF	IOMM2 (read write)
// 0xFFD0 to 0xFFD7	unused
...
//####### IOMM2 ################################################################
	...
	wire[63:0] p3_to_mem;
	wire[63:0] p3_from_mem;
	wire p3_req;
	wire p3_wren;
	wire p3_ready;
	
	assign iomm2_en = (&IO_address[15:6]) & ~IO_address[5] & ~IO_address[4];	//0xFFC0 to 0xFFCF
...
//####### Flash Controller ####################################################
	...
	DE1_flash_controller flash_controller_inst(
		.clk(clk_sys),
		.rst(rst),
		//Port 1
...
//####### SDRAM Controller ####################################################
	...
	SDRAM_DP64_I SDRAM_controller(
			...
			.p2_address(p3_address[12:0]),
			.p2_page(p3_address[19:13]),
			.p2_to_mem(p3_to_mem),
			.p2_from_mem(p3_from_mem),
			.p2_req(p3_req),
			.p2_wren(p3_wren),
			.p2_ready(p3_ready),
